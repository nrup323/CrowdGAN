...
class FudanDataset(BaseDataset):
    def initialize(self, opt):
        ...
        for scene_dir in scene_dirs:
            self.den_paths = sorted(make_dmap_dataset(scene_dir))
            self.img_paths = sorted(make_png_dataset(scene_dir))
            print(f"Length of the img_paths is  : {len(self.img_paths)}")
            assert(len(self.den_paths) == len(self.img_paths))
...
class TestFetcher():
    ...
    def __next__(self):
        if self.index == len(self.dmap_paths) - len(self.img_paths):
            raise StopIteration()
        self.img_path = [os.path.join(self.results_dir, path) for path in os.listdir(self.results_dir)]
        self.img_path.sort()
        self.img_path = self.img_path[self.index:self.index+self.n_frames_G-1]
        print(self.img_path)
        dmap_paths = self.dmap_paths[self.index: self.index+self.n_frames_G]
        imgs = dmaps = 0
        for i,img_path in enumerate(self.img_path):
            img = Image.open(img_path)
            img = self.img_transform(img)
            imgs = img if i == 0 else torch.cat([imgs, img], dim=0)
            if i == len(self.img_path) - 1:
        ...
        self.index += 1
        return {'D': dmaps, 'I': imgs, 'D_path': dmap_paths[-1], 'I_path': self.img_path[-1]}
